...
export async function fetchReverseGeocode(lat, lon) {
    ...
    const u = new URL(`${haUrl}/api/life180/reverse_geocode`);
...
export async function fetchConfig() {
    const url = `${haUrl}/api/life180/config`;
    try {
        const data = await fetchData(url);
        return data;
    } catch (error) {
...
export async function fetchAdmin() {
    const url = `${haUrl}/api/life180/is_admin`;
    try {
        const data = await fetchData(url);
...
export async function fetchPersons() {
    const url = `${haUrl}/api/life180/persons`;
    try {
        const data = await fetchData(url);
...
export async function fetchZones() {
    const url = `${haUrl}/api/life180/zones`;
    try {
        const data = await fetchData(url);
        await setZones(data);
    } catch (error) {
...
export async function fetchFilteredPositions(person_id, startDate, endDate) {
    ...
    const url = `${haUrl}/api/life180/filtered_positions?person_id=${encodeURIComponent(person_id)}&start_date=${encodeURIComponent(startDate)}&end_date=${encodeURIComponent(endDate)}`;
