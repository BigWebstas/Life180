## Life180 — Instrucciones rápidas para agentes de IA

Objetivo: dar a un agente de IA la información mínima y accionable para trabajar en este repositorio.

- Arquitectura general
  - Integración de Home Assistant en `custom_components/life180` (servidor Python) y una UI estática en `custom_components/life180/www` (frontend JS/HTML/CSS).
  - Backend expone endpoints REST bajo `/api/life180/*` (ver `custom_components/life180/api/*.py`). Ejemplos: `/api/life180/config`, `/api/life180/devices`, `/api/life180/persons`.
  - La integración registra recursos Lovelace y un panel en tiempo de ejecución (ver `__init__.py`): sirve estáticos en `/life180` y añade panel con `embed_iframe=True`.
...
- Frontend / desarrollo UI
  - Archivos estáticos en `www/`. `index.html` carga en modo debug `./js/main.js` y en producción `./dist/life180.js` (detectado por la query `debug` y el parámetro `v`).
...
- Cómo encontrar y cambiar comportamiento del servidor
  - Endpoints y lógica: `custom_components/life180/api/*.py` (cada endpoint es una clase `HomeAssistantView`). Modificar aquí para cambiar contratos REST.
...
- Workflows de desarrollador (detectables desde el repo)
  ...
  - Para iterar en la UI: abrir `https://<HA_HOST>/life180/index.html?debug=1&v=<timestamp>` para cargar los archivos `www/js/*.js` sin bundle y forzar cache-bust.
...
Si quieres, actualizo o acorto secciones concretas (por ejemplo, añadir comandos de build si me indicas cómo se genera `dist/life180.js`).
